...
Bring **all six branches** to the session pre-synthesised (bitstreams on a
...
| `testing_i2c7` | `testing_i2c6` + 518bd0c | Imported Scott Larson's reference design from Digi-Key's eewiki: `i2c_master.vhd` (v2.2) and `pmod_temp_sensor_adt7420.vhd`, both unchanged from source. New thin wrapper `adt7420_reader_digikey.vhd` inverts btnc → reset_n, instantiates the pmod reader at 100 MHz, converts the raw 16-bit register value (16-bit mode, LSB=0.0078125 °C) to signed tenths of a degree, and pulses `temp_valid` on each new reading. `led(0)` driven by Larson's `i2c_ack_err`. SCL bus speed left at Larson's default of 400 kHz. | **Build F** — gold-standard reference design. Free-running `data_clk`, two-phase SDA write/read (mid-SCL-low / mid-SCL-high), native repeated START via address-change, supports clock stretching. Used in countless production projects since 2012. |

A note on the histories: Builds A → B → C → D → E → F form a linear
progression off `testing_Sr` where each layer eliminates one more
variable. Build A2 is a parallel attempt rooted at the original
`1efb18f`, with a different theory about where the bug is.

### Why Build F is qualitatively different from everything before it

A–E share the same approach: we wrote our own master from scratch (or
adapted aslak3's). Every regression has been a subtle interplay between
how SCL transitions are timed and where state-machine transitions
happen relative to those edges.

Larson's `i2c_master.vhd` does it differently: a **free-running 4×
divider** generates `scl_clk` and `data_clk` continuously. The state
machine writes SDA on `data_clk`'s rising edge (mid-SCL-low — full
setup time) and reads SDA on `data_clk`'s falling edge (mid-SCL-high
— slave is firmly stable). **There are no FSM transitions near SCL
edges**, so the entire class of timing bugs we've been chasing simply
cannot happen.

It also handles things our designs don't: clock stretching (slave can
hold SCL low to pause the master), repeated START via just changing
the address mid-transaction, proper SDA-glitch suppression during
START/STOP. Production-quality.

If Build F also shows `FF FF`, the problem is definitively **not**
protocol or timing — it's the FPGA pin chain itself (IOBUF
synthesis, pullups, or external connection).
...
- **testing_i2c7** only:
  - `thermostat_top.vhd` instantiates `adt7420_reader_digikey` —
    Larson's stack lives behind that wrapper. `i2c_controller` should
    NOT appear in the elaborated design
  - All three Digi-Key files present: `i2c_master.vhd`,
    `pmod_temp_sensor_adt7420.vhd`, `adt7420_reader_digikey.vhd`
  - In `pmod_temp_sensor_adt7420.vhd` line ~67: `bus_clk => 400_000`
    (this is Larson's default; if you want safer 100 kHz, change here)
  - `reset_n <= not reset` inside the wrapper — reset polarity is
    flipped, so btnc still works as the active-high reset button
  - `led(0)` is driven by Larson's `i2c_ack_err` — should stay LOW
    when transactions succeed, go HIGH if any byte gets NAKed

---

## Flash order and decision tree

Start with **Build F (testing_i2c7)** — battle-tested production
reference design. If it works, you're done; if it doesn't, the
cause is physical (pullups / IOBUF / sensor) and no FSM rewrite
will help. Fall back through E → D → C → B → A only if F fails.

```
 Flash F ──► scope shows S 4BW~a 00~a Sr 4BR~a <MSB>a <LSB>~a P
   │        with real temperature bytes?
   │
   ├── YES → You're done. Confirm 7-seg shows plausible temp.
   │        led(0) (i2c_ack_err) should stay LOW throughout.
   │
   └── NO ──► What's on the scope?
             │
             ├── led(0) lit → some byte got NAKed. Probe address byte
             │   carefully on scope: confirm 0x4B = 1001011. If wrong,
             │   sensor jumpers may differ from Nexys A7 default
             │
             ├── No SCL clocks at all → reset_n still asserted? Wrapper
             │   inverts btnc; if btnc held HIGH at boot, sensor sees
             │   reset and never starts. Probe scl pin directly
             │
             ├── S 4BW~a NAK on first byte → wrong address. Try
             │   "1001000" (0x48) by changing SENSOR_ADDR generic
             │
             └── S 4BW~a 00~a Sr 4BR~a FF FF P → slave ACKs everything
                 but no data driven. **This is the diagnostic outcome**:
                 since Larson's master is known good, the FPGA-side
                 IOBUF or pullup is broken. Open implemented design,
                 verify TMP_SDA routes to a real IOBUF primitive
                 (not OBUFT+IBUF split). Check XDC for `PULLUP true`
 │
 ▼
 Fall back to Build E (testing_i2c6) — single-FSM reader. If F fails
 with bus activity but E shows clean activity, something in Larson's
 stack disagrees with our pin chain (very unlikely)
 │
 ▼
...
### Build F (Larson — pointer-write + repeated START path, 16-bit mode)

```
 S  4BW~a  03~a  80~a  P    (set_resolution: write 0x80 to config reg 0x03)
 S  4BW~a  00~a  Sr 4BR~a  <MSB>~a  <LSB>~a  P    (read temp, repeating)
```

- The first transaction (config write to 0x03) only happens once after
  reset — set 16-bit resolution mode
- Subsequent transactions are pointer-write + repeated START + 2-byte
  read, repeating ~every 1.3 μs (Larson's pause is intentionally short)
- 16-bit mode: MSB ≈ `0x0B`, LSB ≈ `0x00..0xFF` (no status flags here,
  full 16-bit signed temperature with LSB = 0.0078125 °C)
- For 22 °C: raw = 22/0.0078125 = 2816 = `0x0B00`. Wrapper converts to
  220 (tenths) before reaching `sig_temp_vector`
- `led(0)` driven by `i2c_ack_err`: should stay OFF entirely; lit
  means something NAKed
